...
pkgs.callPackage ../../metapkgs/combine.metapkg.nix { pack = [
  ...
  in "${pkgs.writeScriptBin "setWallpaper" ''
    ...
    sed 's~Exec=kitty~Exec=${pkgs.callPackage ../../../templates/packages/kitty.pkg.nix { self = pkgs; }}/bin/kitty~' ~/.nix-profile/share/applications/kitty.desktop > ~/.local/share/applications/kitty.desktop
    '' else ""}
    while true;
