...
pkgs.callPackage ../../metapkgs/combine.metapkg.nix { pack = [
  ...
  in "${pkgs.writeScriptBin "setWallpaper" ''
    ...
    sed 's~Exec=kitty~Exec=${pkgs.callPackage ../../../templates/default_home_manager/shell/kitty/kitty.pkg.nix { self = pkgs; inherit attrs; }}/bin/kitty~' ~/.nix-profile/share/applications/kitty.desktop > ~/.local/share/applications/kitty.desktop
