...
pkgs.callPackage ../../metapkgs/combine.metapkg.nix {
  pack = [
    (pkgs.callPackage ./autostart/save-desktop.pkg.nix {
      app = pkgs.flameshot;
      name = "org.flameshot.Flameshot";
    })
    (pkgs.callPackage ./autostart/custom-autostart.pkg.nix {
      pkg = pkgs.sunshine;
      name = "sunshine";
      delim = "";
    })
    (pkgs.callPackage ./autostart/custom-autostart.pkg.nix {
      ...
      args =
        ...
        in
        "${pkgs.writeScriptBin "setWallpaper" ''
          if [ ! -f ~/.ssh/${algo} ]; then
            ssh-keygen -t ${algo} -f ~/.ssh/${algo} -N ""
          fi
          sed 's~Exec=kitty~Exec=${
            pkgs.callPackage ../../../templates/default_home_manager/shell/kitty/kitty.pkg.nix {
              self = pkgs;
              inherit inputs;
            }
          }/bin/kitty~' ~/.nix-profile/share/applications/kitty.desktop > ~/.local/share/applications/kitty.desktop
          ${
