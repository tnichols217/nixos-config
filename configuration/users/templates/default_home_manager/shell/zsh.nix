        ...
        initContent =
          let
            functions = {
              ...
              upgf = "sudo ${pkgs.bash}/bin/bash -c \"cd /etc/nixos; ${pkgs.git}/bin/git stash; ${pkgs.git}/bin/git stash clear; ${pkgs.git}/bin/git pull; ${pkgs.nixos-rebuild-ng}/bin/nixos-rebuild switch --flake \\\".#${host-name}\\\" \\\"\$@\\\" --refresh --log-format internal-json |& ${pkgs.nix-output-monitor}/bin/nom --json\"";
            };
          in
